...
over the same tables. Three commands: `discover` -> `extract` -> `rill` (optional).
...
- `src/commands/shield/events/{discover,extract,rill}.ts` — the oclif commands. Each is a
...
- `src/events/{consolidate,logfiles,project,rill,stream,tables}.ts` — modules that know nothing
...
- `CHANGELOG.md` follows Keep a Changelog; the public surface is the three commands and their flags.
